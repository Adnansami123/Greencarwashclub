...
import {
  useAddAssetMutation,
  usePutAssetMutation,
} from "../../store/AssetsAPI/AssetsAPI";
import {
...
export default function CompanyModal({
  ...
  useEffect(() => {
    if (!!stateData) {
      ...
      dispatch(
        cartStateActions.setUnderUserXid({ UnderUserXid: stateData.userXid })
      );
    }
  }, [stateData]);
  const initialStates = {
    ...
    if (URLPathName === "AddCompany") {
      setStates({
        ...
        form: {
          ...
          showArabicCompanyField:
            Strings.company.company.form.showArabicCompanyField,
  ...
  const [
    ...
    {
      data: getSendRegistrationEmailData,
      isSuccess: isSendRegistrationEmailSuccess,
    },
  ] = useSendRegistrationEmailMutation();
  ...
  useEffect(() => {
    if (isAddAssetSuccess && !!getAddAssetData) {
      if (getAddAssetData?.statusCode === 500) {
        ...
        const data = {
          id: getAddAssetData?.details,
        };

        SendRegistrationEmail({ data: data });
      }
    }
  ...
  const [
    AddAssetImages,
    { data: getAddAssetImageData, isSuccess: isAddAssetImagesSuccess },
      ...
      if (!!allFilesAadhaar?.file || !!allFilesPAN?.file) {
        ...
        if (!!allFilesAadhaar?.file) {
          allFilesAadhaar?.file?.fileList.forEach((file, index) => {
            bodyFormSetData.append(
              "FileDetails[" + customIndex + "].TypeOfDoc",
              file.type
            );
            bodyFormSetData.append(
              "FileDetails[" + customIndex + "].OriginalName",
              file.name
            );
            bodyFormSetData.append(
              "FileDetails[" + customIndex + "].ImagePath",
              ""
            );
            bodyFormSetData.append(
              "FileDetails[" + customIndex + "].DocExtension",
              file.type
            );
            bodyFormSetData.append(
              "FileDetails[" + customIndex + "].ExistingFileName",
              file.name
            );
            bodyFormSetData.append(
              "FileDetails[" + customIndex + "].FileNameGuid",
              file.uid + getExtensionByMIMEType({ fileName: file.type })
            );
            bodyFormSetData.append(
              "FileDetails[" + customIndex + "].Blob",
              file.originFileObj
            );
    ...
    if (addorUpdateImages) {
      allFilesAadhaar.file.fileList.forEach((el, index) => {
        ...
        if (el?.isOld !== true) {
          ...
          AssetImagesObj.savedName =
            el.uid +
            getExtensionByMIMEType({
              fileName: allFilesAadhaar.file.fileList[index].type,
            });
      ...
      allFilesPAN.file.fileList.forEach((el, index) => {
        ...
        if (el?.isOld !== true) {
          ...
          AssetImagesObj.savedName =
            el.uid +
            getExtensionByMIMEType({
              fileName: allFilesPAN.file.fileList[index].type,
            });
          AssetImagesObj.originalName = el.name;
          AssetImagesObj.imagePath = "string";
          AssetImagesObj.docExtension = el.type;
  ...
  const [putAsset, { data: getPutAssetData, isSuccess: isPutAssetSuccess }] =
    usePutAssetMutation();

  useEffect(() => { }, [getPutAssetData]);

  const [
    fetchBranches,
    { data: getBranchesData, isSuccess: isGetBranchesSuccess },
  ] = useBranchByCountryIDMutation();

  const [
    fetchStateNames,
    { data: getStateNamesData, isSuccess: isGetStateNamesSuccess },
  ] = useStateNamesMutation();
  ...
  const onFinish = (values) => {
    ...
    const commonCompany = {
      User: {
        ...
        password: "test11",
        lastEditByXid: 9999,
        // NewUnderUserXid: stateData.userXid,
        // NewUnderUserXid: stateData.userXid,  /// should be from the code behind...
        // ReferralCode: form.getFieldValue("ReferralCode"),
        // only referral code mapping here...
        //form.getFieldValue("UnderUserXid"), // this is under user id.....
      },
      CompanyBranchBank: {
        BankName: "",
        AccountNo: "",
  ...
  const onClickReset = () => {
    ...
    // need to check the confirmation then redirect....
  ...
  const OnRemoveHandler = (e) => {
    if (e.type == "aadhar") {
      const filterFiles = filterRemovedFiles({
        data: allFilesAadhaar,
        removeUID: e.e.file.file.uid,
      });
  ...
  useEffect(() => {
    if (isSendRegistrationEmailSuccess) {
      dispatch(setLoadingModalConfiguration({ isVisible: false }));
      setshowActionMessage(true);
    }
  }, [isSendRegistrationEmailSuccess]);

  const OnSuccessHandlerRegister = () => {
    //here updating the details.....
    dispatch(
      cartStateActions.setUserXID({ UserXid: getAddAssetData?.details })
    );
    //after successful...
  ...
  const onChangeCountry = (e) => {
    form.setFieldsValue({ branchXid: '' });
    fetchBranches({ countryid: e, });
  }
      ...
      <div class="container">
        ...
        >
          ...
          <>
            {error ? (
              <>
                <div>Error occurred : {errorMessage}</div>
              </>
            ) : null}
                ...
                <div className="row register-form">
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      >
                        <Radio.Group
                          // onChange={(e) => setShow4Copies(e.target.value)} 
                          defaultValue={"F"}>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      >
                        <Select placeholder="Company Country" onChange={onChangeCountry}>
                  ...
                  {/* <div class="col-md-6">
                    <div class="form-group">
                      ...
                      >
                        <Input
                          readOnly={!!stateData?.referralCode ? true : false}
