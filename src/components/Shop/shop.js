...
export default function Shop({
  ...
  const handleColorChange = (color) => {
    setSelectedColors((prev) => {
      // If "All Colors" is clicked
      if (color === "All Colors") {
        // If "All Colors" is already selected, deselect it
        if (prev.includes("All Colors")) {
          return [];
        }
        // Otherwise, select only "All Colors" and deselect everything else
        return ["All Colors"];
      } else {
        // If anything else is clicked, we first remove "All Colors" from selection
        const newColors = prev.filter((col) => col !== "All Colors");

        // Then toggle the clicked color
        if (newColors.includes(color)) {
          return newColors.filter((col) => col !== color);
        } else {
  ...
  const filteredProducts = displayedProducts?.filter((product) => {
    // Check if the product passes category filter
    const passesCategory =
      selectedCategories.length === 0 ||
      selectedCategories.includes("All Categories") ||
      selectedCategories.includes(product.nameEng);

    // Check if the product passes price filter
    const passesPrice =
      product.salePrice >= priceRange[0] && product.salePrice <= priceRange[1];

    // Check if the product passes color filter
    const passesColor =
      selectedColors.length === 0 ||
      selectedColors.includes("All Colors") ||
      // Look for the selected color in item specifications
      (product.itemSpecification &&
        product.itemSpecification.some((spec) =>
          selectedColors.includes(spec.nameEng),
        ));

    // Return true only if product passes all filters
    return passesCategory && passesPrice && passesColor;
  });
  ...
  const colors = React.useMemo(() => {
    if (!products || products.length === 0) return ["All Colors"];
    ...
    products.forEach((product) => {
      product?.itemSpecification?.forEach((spec) => {
        if (spec?.nameEng) {
          colorSet.add(spec.nameEng);
        }
      });
    });

    return ["All Colors", ...Array.from(colorSet)];
  }, [products]);

  // Dynamic image handling for any product
  const getProductImage = (product, isHover = false) => {
    if (!product || !product.itemSpecification || product.itemSpecification.length === 0) {
