...
export default function Shop({
  ...
  const handleAddToCart = (e, product) => {
    ...
    const existingCartItem = cart.find((item) => item.id === product.pid);
    const currentQuantityInCart = existingCartItem
      ? existingCartItem.quantity
      : 0;

    // Check if adding one more would exceed available stock
    if (currentQuantityInCart >= product.availableStock) {
      // Show popup message - you can implement this using a state or a toast library
      alert(`Sorry, only ${product.availableStock} items available in stock.`);
      return;
    }
  ...
  const getProductImage = (product, isHover = false) => {
    if (!product || !product.itemImages || product.itemImages.length === 0) {
      return BlankImage;
    }

    // For hover, use second image if available, otherwise use first image
    const imageIndex = isHover && product.itemImages.length > 1 ? 1 : 0;
    return `https://amsfilecontainercc.blob.core.windows.net/uatfilecontainer/${product.itemImages[imageIndex]?.savedName}`;
  };
          ...
          <div className="w-full md:w-2/3 lg:w-3/4">
            ...
            <div className="mb-6 sm:mb-8 bg-gray-400 rounded-lg p-4 sm:p-6 lg:p-8 text-center relative overflow-hidden">
              <div className="relative z-10">
                ...
                <button className="bg-gradient-to-r from-purple-500 via-blue-500 to-indigo-500 text-white px-4 sm:px-6 py-1.5 sm:py-2 rounded-md transition-all text-xs sm:text-sm hover:scale-110 hover:from-purple-600 hover:via-blue-600 hover:to-indigo-600">
                  Shop Now
                </button>
              </div>
                    ...
                    <div className="absolute inset-0 bg-opacity-20 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end z-20 pointer-events-none">
                      ...
                      <div className="px-2 py-2 sm:py-3 bg-white bg-opacity-90 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex flex-col gap-2 relative z-30 pointer-events-auto">
                        {product.availableStock === 0 ? (
                          <button
                            className="w-full py-1.5 sm:py-2 bg-red-600 text-white font-medium rounded-md cursor-not-allowed text-xs sm:text-sm"
                            disabled
                          ...
                          <button
                            className="w-full py-1.5 sm:py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-indigo-700 hover:to-purple-700 text-white font-medium rounded transition-all duration-300 text-xs sm:text-sm flex items-center justify-center gap-1 sm:gap-2"
                            onClick={(e) => {
                              e.stopPropagation();
                      ...
                      <div className="mt-2 sm:mt-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {!product.availableStock ? (
                            ...
                            <button
                              className="flex-1 py-1.5 sm:py-2 bg-black hover:bg-pink-500 text-white font-medium rounded-md transition-colors text-xs sm:text-sm"
                          ...
                          <button
                            ...
                            onClick={(e) => toggleWishlist(e, product.pid)}
                          >
                            <Heart
                              size={16}
                              className={
                          ...
                          <button className="p-1.5 sm:p-2 border border-gray-300 rounded-md hover:bg-gray-50">
