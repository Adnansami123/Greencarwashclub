...
import { getFilterByAccountHead, getFilterByConsineePid, getFilterByExpernses, getFilteredEMandates } from "../../utils";
import {
    DownloadOutlined, PlusCircleOutlined,
    FormOutlined,
    DeleteOutlined,
} from '@ant-design/icons';
import { useGetPaymentMasterMutation, useGetPaymentTypesMutation } from "../../store/PaymentTypesAPI/PaymentTypesAPI";
export default function PaymentMasterSelect({
    onSuccess,
    inputName = "",
    labelName = "",
    placeHolderName = "",
    ...
    useEffect(() => {
        if (getFetchClientsData?.length > 0) {
            ...
            setClientsData(getFilterByExpernses({ data: getFetchClientsData, isExpenses: isExpenses }));
